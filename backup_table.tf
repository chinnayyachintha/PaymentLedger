...
resource "aws_backup_vault" "payment_ledger_vault" {
  name         = "${var.dynamodb_table_name}-backup-vault"
  kms_key_arn  = aws_kms_key.payment_ledger_key.arn # KMS key for encryption
}


# AWS Backup Plan for daily backups of DynamoDB table
resource "aws_backup_plan" "payment_ledger_backup_plan" {
  name = "${var.dynamodb_table_name}-backup-plan"

  rule {
    rule_name         = "daily-backup"
    schedule          = "cron(0 0 1 * ? *)"  # 1st day of every month at midnight UTC
    target_vault_name = aws_backup_vault.payment_ledger_vault.name  # Corrected argument name
...
resource "aws_backup_selection" "payment_ledger_backup_selection" {
  ...
  plan_id      = aws_backup_plan.payment_ledger_backup_plan.id
  iam_role_arn = aws_iam_role.backup_role.arn  # IAM Role for Backup Selection
  resources = [
    aws_dynamodb_table.payment_ledger.arn
  ]
}
